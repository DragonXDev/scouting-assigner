...
people = ["Alice", "Bob", "Charlie", "David", "Emily", "Frank", "Grace", "Henry", "Isabella", "Jack", "Katie", "Liam", "Mia", "Noah", "Olivia", "Ethan", "Ava", "Sophia"]  # Replace with your list of people
...
def assign_scouting(formatted_matches, people):
    ...
    def is_valid_assignment(person, team, match_index, formatted_matches):
        ...
        for future_match_index in range(match_index, len(formatted_matches)):
            ...
            for existing_team in assignments[person]:
                if any(existing_team in alliance for alliance in future_match) or any(team in alliance for alliance in future_match):
                    return False
        return True
            ...
            for team in alliance:
                if team not in team_assignments:
                    for person in people:
                        if person not in assigned_people_in_match and slots_remaining[person] > 0 and is_valid_assignment(person, team, match_index, formatted_matches):
...
def resolve_duplicates(scouting_assignments, matches):
        ...
        for team in teams:
            for match in matches:
                if team in match['alliances']['red']['team_keys'] or team in match['alliances']['blue']['team_keys']:
                    ...
                    match_detail = f"{comp_level}{match_number} + {team}"  # Add team context
